  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // auto redirect si 1 org
        if (data.organizations?.length === 1) {
          handleSelect(data.organizations[0])
        }
      } catch {
   ...
   */
    ...
    const redirectByRole = (role: string, slug?: string) => {
      switch (role) {
        ...
        case "MEMBER":
        default:
          router.push(`/${slug}/member/dashboard/${slug}`)
          break
      }
    }
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 text-sm text-white/60">
        <Sparkles size={14} />
        Espace sécurisé multi-organisation
      </div>

      <h1 className="text-3xl font-bold mt-2">
        Choisis ton organisation
      </h1>

      <p className="text-white/50 mt-2 text-sm">
        Chaque organisation est isolée avec ses données, rôles et permissions.
      </p>
    </motion.div>
  )
}
  ...
  return (
    <div className="relative">
      ...
      <input
        ...
        className="w-full pl-10 pr-4 py-3 rounded-xl bg-white/5 border border-white/10 focus:ring-2 focus:ring-white/20 outline-none"
      />
    </div>
  )
}
...
function OrgCard({
  ...
  return (
    <motion.div
      ...
      onClick={onClick}
      className={`flex justify-between items-center p-4 rounded-2xl cursor-pointer border transition
...
function Footer() {
  return (
    <div className="text-center mt-12 text-xs text-white/30">
      © {new Date().getFullYear()} Assopilot — Enterprise SaaS Multi-Tenant
    </div>
  )
}
